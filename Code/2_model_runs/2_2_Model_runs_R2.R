...
for(survey.i in 1:9)
{
  ...
  R1 <- read.csv(paste("2_model_runs/Results/Survey_", survey.i, "_", Study.ID$References[survey.i], "_R1.csv", sep = ""))
  ...
  {
    write.csv(result.sens.real, paste("2_model_runs/Results/Survey_", survey.i, "_", Study.ID$References[survey.i], "_R2_FINAL.csv", sep = ""), row.names = FALSE)
  }
  
  if(pass.no >= 800 & pass.no < 1000)   # continue sampling till 1000
  {
    # Rerun the model with a modified sampler
    results.2 <- mclapply(1:nc, function(i){
      simulate.fun(n.species, (1000 - pass.no)/pass.no *1.2 * 21000, i, sample.function2, keep.k = n.species-1)  # set the second par: the number of repetitions to a smaller value if you want to try the code
    }, mc.cores = nc)
    ...
    write.csv(rbind(result.sens.real, result.sens.real.2), paste("2_model_runs/Results/Survey_", survey.i, "_", Study.ID$References[survey.i], "_R2_FINAL.csv", sep = ""), row.names = FALSE)
  }

  if(pass.no >= 50 & pass.no < 800)   # same the results for the next round
  {
    write.csv(result.sens.real, paste("2_model_runs/Results/Survey_", survey.i, "_", Study.ID$References[survey.i], "_R2.csv", sep = ""), row.names = FALSE)
  }
  
  if(pass.no < 50) # cut beta again and rerun
  {
    beta.range <- beta_for_R2(result.sens.real, Animal_prev, 0, 2)   # use the function beta_for_R2() to obtain the new ranges for betas
    print(beta.range)
    
    # Rerun the model with a modified sampler
    results <- mclapply(1:nc, function(i){
      simulate.fun(n.species, 21000, i, sample.function2, keep.k = n.species-1)  # set the second par: the number of repetitions to a smaller value if you want to try the code
    }, mc.cores = nc)
    ...
    if(pass.no < 50)
    {
      # Rerun the model with a modified sampler
      results.2 <- mclapply(1:nc, function(i){
        simulate.fun(n.species, (50 - pass.no)/pass.no *1.2 * 21000, i, sample.function2, keep.k = n.species-1)  # set the second par: the number of repetitions to a smaller value if you want to try the code
      }, mc.cores = nc)
      ...
      write.csv(rbind(result.sens.real, result.sens.real.2), paste("2_model_runs/Results/Survey_", survey.i, "_", Study.ID$References[survey.i], "_R2.csv", sep = ""), row.names = FALSE)
    }
  }
}
